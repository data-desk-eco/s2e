...
pub fn snap(x: f64) -> f64 { (x / GRID_STEP).round() * GRID_STEP }
...
pub fn cell_key(lon: f64, lat: f64) -> String { format!("{:.3},{:.3}", snap(lon), snap(lat)) }
...
pub fn grid_sites(bbox: [f64; 4]) -> Vec<Site> {
    ...
    for j in k(s)..=k(n) {
        ...
        for i in k(w)..=k(e) {
            ...
            sites.push(Site { h3: cell_key(lon, lat), lon, lat });
        }
    }
    sites
}
...
mod tests {
    ...
    fn grid_and_key() {
        ...
        let mut hist = [0u32; N_SCL]; hist[6] = 21; hist[3] = 1; hist[8] = 1; hist[9] = 1; hist[10] = 1;
        let r = CoverRow { h3: "x".into(), px_valid: 25, hist };
