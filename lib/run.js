...
export async function runAOI(bbox, start, end, opts = {}) {
    const { thresholds, concurrency = 4, maxCloudCover = 100, signal,
            cluster = {}, onEvent, store, source = 'aws', detect = detectImage } = opts;

    const items = [];
    for await (const it of searchSTAC(bbox, start, end, { maxCloudCover, signal, source })) items.push(it);
        ...
        while (idx < items.length) {
            ...
            try {
                const r = store ? await store(item, signal)
                                : await detect(item, bbox, { thresholds, signal });
