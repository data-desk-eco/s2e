...
impl Default for Opts {
    fn default() -> Self {
        Opts { merge_distance: 135.0, min_dates: 1, min_avg_b12: 0.5, observations: None, score_threshold: 0.0 }
    }
}
...
pub fn cluster_js(detections: JsValue, opts: JsValue) -> Result<JsValue, JsValue> {
    ...
    let clusters = cluster_detections(&dets, &ClusterOptions {
        merge_distance: o.merge_distance, min_dates: o.min_dates, min_avg_b12: o.min_avg_b12,
        observations: o.observations, score_threshold: o.score_threshold,
    });
...
pub fn score_cluster_js(max_ratio: Option<f64>, n_dates: f64, n_obs: f64, min_glint: Option<f64>) -> Vec<f64> {
    let s = s2_flares_core::score_cluster(max_ratio, n_dates, n_obs, min_glint);
    vec![s.ratio_score, s.persistence_score, s.glint_penalty, s.total_score]
}
