...
pub fn decode_bands(b12_url: &str, b11_url: &str, b8a_url: Option<&str>)
    ...
    if let Some(ref a) = b8a { streams.push(a); }
...
mod parity {
    ...
    fn bulk_matches_windowed() {
        ...
        let win = GdalReader { harmonize: true };
        let bulk_cpu = BulkReader { gpu: false, harmonize: true };
        let bulk_gpu = BulkReader { gpu: true, harmonize: true };
        for it in &items {
            let w = detect_scene(&win, it, bbox, false, &t, false).expect("windowed detect").0;
            let c = detect_scene(&bulk_cpu, it, bbox, false, &t, false).expect("bulk-cpu detect").0;
            let g = detect_scene(&bulk_gpu, it, bbox, false, &t, false).expect("bulk-gpu detect").0;
    ...
    fn same(a: &[Detection], b: &[Detection], who: &str, it: &crate::stac::Item) {
        assert_eq!(a.len(), b.len(), "{who} det count mismatch {} {}", it.mgrs, it.date);
        for (x, y) in a.iter().zip(b.iter()) {
            assert_eq!(
                (x.lon.to_bits(), x.lat.to_bits(), x.max_b12.to_bits(), x.avg_b12.to_bits(), x.pixels),
                (y.lon.to_bits(), y.lat.to_bits(), y.max_b12.to_bits(), y.avg_b12.to_bits(), y.pixels),
                "{who} detection mismatch {} {}", it.mgrs, it.date);
        }
    }
...
mod bench {
    ...
    fn bench_readers() {
        ...
        let (start, end) = (go("S2_BENCH_START", "2024-08-01"), go("S2_BENCH_END", "2024-08-31"));
        let n: usize = go("S2_BENCH_N", "3").parse().unwrap();
        let mut items = crate::stac::search(bbox, &start, &end, 100.0, "cdse").expect("stac search");
        if let Ok(tile) = std::env::var("S2_BENCH_TILE") { items.retain(|i| i.mgrs == tile); }
        items.truncate(n);
        assert!(!items.is_empty(), "no scenes for the bench bbox/window");
        let t = Thresholds::default();
        let readers: [(&str, Box<dyn SceneReader>); 3] = [
            ("windowed", Box::new(GdalReader { harmonize: true })),
            ("bulk-cpu", Box::new(BulkReader { gpu: false, harmonize: true })),
            ("bulk-gpu", Box::new(BulkReader { gpu: true, harmonize: true })),
        ];
        ...
        for (i, (name, _)) in readers.iter().enumerate() {
            eprintln!("  {name:8} {:.1}s  ({:.1}s/tile)", tot[i], tot[i] / items.len() as f64);
        }
        eprintln!("speedup vs windowed: bulk-cpu {:.2}× · bulk-gpu {:.2}×", tot[0] / tot[1], tot[0] / tot[2]);
    }
    ...
    fn bench_throughput() {
        ...
        assert!(!items.is_empty(), "no scenes for the bench bbox/window");
        let t = Thresholds::default();
        let pool = rayon::ThreadPoolBuilder::new().num_threads(c).build().unwrap();
        let readers: [(&str, Box<dyn SceneReader>); 3] = [
            ("windowed", Box::new(GdalReader { harmonize: true })),
            ("bulk-cpu", Box::new(BulkReader { gpu: false, harmonize: true })),
            ("bulk-gpu", Box::new(BulkReader { gpu: true, harmonize: true })),
        ];
        eprintln!("throughput: {} scenes · concurrency {} · {}..{}", items.len(), c, start, end);
        let mut tot = [0.0f64; 3];
        for (i, (name, r)) in readers.iter().enumerate() {
            use rayon::prelude::*;
            let t0 = Instant::now();
            let dets: usize = pool.install(|| items.par_iter()
                .map(|it| detect_scene(&**r, it, it.bbox, true, &t, false).map(|(d, _)| d.len()).unwrap_or(0))
                .sum());
            tot[i] = t0.elapsed().as_secs_f64();
            eprintln!("  {name:8} {:.1}s  ({:.1}s/scene · {:.1} scenes/min · {} det)",
                tot[i], tot[i] / items.len() as f64, items.len() as f64 * 60.0 / tot[i], dets);
        }
        eprintln!("throughput speedup vs windowed: bulk-cpu {:.2}× · bulk-gpu {:.2}×", tot[0] / tot[1], tot[0] / tot[2]);
    }
}
