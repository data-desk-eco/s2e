...
// proj:epsg, but the tile encodes it: zone = leading digits, hemisphere from the
// latitude band (C–M south, N–X north). 326xx north, 327xx south.
export function epsgFromMgrs(grid) {
    const t = String(grid).replace(/^MGRS-/, '');
    const zone = parseInt(t, 10);
    return (t[String(zone).length] >= 'N' ? 32600 : 32700) + zone;
}
...
// --- bbox helpers (shared by the CLI and the web API) ------------------------

// Bounding box [W,S,E,N] of any GeoJSON geometry (Point / Polygon / Multi*).
...
export function padBbox([w, s, e, n], km) {
...
export function bboxAreaKm2([w, s, e, n]) {
    return (n - s) * 111 * (e - w) * 111 * Math.cos((s + n) / 2 * Math.PI / 180);
}
